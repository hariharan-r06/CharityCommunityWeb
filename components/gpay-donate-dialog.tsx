...
import { Heart, CreditCard, Smartphone, Share2, CheckCircle, AlertCircle } from "lucide-react"
import { Badge } from "@/components/ui/badge"
import { getCharityPaymentDetails } from "@/services/apiService"
import QRCode from "qrcode.react";

interface GpayDonateDialogProps {
  open: boolean
...
export function GpayDonateDialog({ open, onOpenChange, charity, charityId }: GpayDonateDialogProps) {
  ...
  const fetchPaymentDetails = async () => {
    ...
    try {
      const paymentDetails = await getCharityPaymentDetails(charityId);
      
      if (paymentDetails.gpayNumber) {
        setGpayNumber(paymentDetails.gpayNumber);
      } else {
        setError("This charity hasn't set up GPay payments yet.");
      }
    } catch (err) {
  ...
  // Generate UPI URL whenever amount or gpayNumber changes
  useEffect(() => {
    const finalAmount = amount === "custom" ? customAmount : amount;
    if (gpayNumber && finalAmount) {
      // Create UPI deep link format
      // Format: upi://pay?pa=UPI_ID&pn=NAME&am=AMOUNT&cu=CURRENCY&tn=MESSAGE
      const upiLink = `upi://pay?pa=${gpayNumber}&pn=${encodeURIComponent(charity)}&am=${finalAmount}&cu=INR&tn=${encodeURIComponent(`Donation to ${charity}`)}`;
      setUpiUrl(upiLink);
    }
  }, [gpayNumber, amount, customAmount, charity]);

  const handleDonate = () => {
    if (!gpayNumber) {
      setError("Payment method not available. Please try a different option.");
      return;
    }

    setIsProcessing(true);

    // For UPI/GPay payments, open the UPI link
    if (paymentMethod === "upi" || paymentMethod === "paytm") {
      // Open the UPI URL in a new window - this will trigger the UPI app on mobile devices
      window.open(upiUrl, "_blank");
      
      // Since we can't track the completion of external UPI payments automatically,
      // we'll show the success state after a delay to simulate completion
      setTimeout(() => {
          ...
          <div className="space-y-4 py-4">
            <Tabs defaultValue="one-time" value={donationType} onValueChange={setDonationType}>
              ...
              <div className="mt-4 space-y-4">
                ...
                {(paymentMethod === "upi" || paymentMethod === "paytm") && gpayNumber && (
                  <div className="mt-4 p-3 bg-muted rounded-lg flex flex-col items-center">
                    <p className="text-sm mb-2 text-center">Scan with any UPI app or GPay</p>
                    <div className="bg-white p-2 rounded-md mb-2">
                      <QRCode value={upiUrl} size={150} level="H" />
                    </div>
                    <p className="text-xs text-muted-foreground">
                      GPay/UPI ID: {gpayNumber}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 w-full p-3 bg-muted rounded-lg text-left">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {paymentMethod === "card" ? "Credit Card" : paymentMethod === "upi" ? "UPI/GPay" : "Paytm"}
                </span>
              </div>
            </div>
        ...
        {!isLoading && !error && !isComplete && (
          <DialogFooter>
            <Button
              ...
              disabled={isProcessing || (amount === "custom" && !customAmount) || !gpayNumber}
            >
